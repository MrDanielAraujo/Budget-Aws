...
import centroCategoriaValidator from './centro-categoria-form-validator.json';

@Component({
  selector: 'app-centro-categoria-form',
  standalone: true,
  ...
  templateUrl: './centro-categoria-form.component.html',
  styleUrls: ['./centro-categoria-form.component.scss']
})
export class CentroCategoriaFormComponent implements OnInit{
  private fb = inject(FormBuilder);
  protected command = inject(Command);
  protected crud = inject(CrudForm<CentroCategoria>);
  @Input() code!: string;
  /**
   * * Esse é o método usado para iniciar o componente.
   */
  ngOnInit(): void {
    this.crud.module = "centroCategoria";
    this.crud.form.errorMessage.validatorMessageForm = centroCategoriaValidator;
