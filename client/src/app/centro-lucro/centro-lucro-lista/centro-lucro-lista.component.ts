...
@Component({
  selector: 'app-centro-lucro-lista',
  standalone: true,
  imports: [CommonModule, HeaderTableComponent, FooterTableComponent, ReactiveFormsModule],
  templateUrl: './centro-lucro-lista.component.html',
  styleUrls: ['./centro-lucro-lista.component.scss']
})
export class CentroLucroListaComponent implements OnInit {
  private fb = inject(FormBuilder);
  protected crud = inject(CrudList);
  protected dataList?: CentroLucro[];
   ...
   */
  ngOnInit(): void {
    this.crud.module = this.crud.sessionKey = "centroLucro";
  ...
  private actionAfterInitList() {
    this.crud.dataChange.subscribe((value: CentroLucro[]) => this.setDataList(value))
  }
  private setDataList(value: CentroLucro[]) {
    this.dataList = value;
