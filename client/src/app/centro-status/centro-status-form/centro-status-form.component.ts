...
import centroStatusValidator from './centro-status-form-validator.json';

@Component({
  selector: 'app-centro-status-form',
  standalone: true,
  ...
  templateUrl: './centro-status-form.component.html',
  styleUrls: ['./centro-status-form.component.scss']
})
export class CentroStatusFormComponent implements OnInit{
  private fb = inject(FormBuilder);
  protected command = inject(Command);
  protected crud = inject(CrudForm<CentroStatus>);
  @Input() code!: string;
  /**
   * Esse é o método usado para iniciar o componente.
   */
  ngOnInit(): void {
    this.crud.module = "centroStatus";
    this.crud.form.errorMessage.validatorMessageForm = centroStatusValidator;
