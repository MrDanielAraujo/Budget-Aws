...
@Component({
  selector: 'app-combo-informacoes-lista',
  standalone: true,
  imports: [CommonModule, AbertoFechadoPipe, HeaderTableComponent, FooterTableComponent, ReactiveFormsModule],
  templateUrl: './combo-informacoes-lista.component.html',
  styleUrls: ['./combo-informacoes-lista.component.scss']
})
export class ComboInformacoesListaComponent implements OnInit {
  private fb = inject(FormBuilder);
  protected crud = inject(CrudList);
  protected dataList?: ComboInformacoes[];
   ...
   */
  ngOnInit(): void {
    this.crud.module = this.crud.sessionKey = "comboInformacoes";
  ...
  private actionAfterInitList() {
    this.crud.dataChange.subscribe((value: ComboInformacoes[]) => this.setDataList(value))
  }
  private setDataList(value: ComboInformacoes[]) {
    this.dataList = value;
