...
@Component({
  selector: 'app-funcionario-dependente-lista',
  standalone: true,
  imports: [CommonModule, AbertoFechadoPipe, HeaderTableComponent, FooterTableComponent, ReactiveFormsModule],
  templateUrl: './funcionario-dependente-lista.component.html',
  styleUrls: ['./funcionario-dependente-lista.component.scss']
})
export class FuncionarioDependenteListaComponent implements OnInit {
  private fb = inject(FormBuilder);
  protected crud = inject(CrudList);
  protected dataList?: FuncionarioDependente[];
   ...
   */
  ngOnInit(): void {
    this.crud.module = this.crud.sessionKey = "funcionarioDependente";
  ...
  private actionAfterInitList() {
    this.crud.dataChange.subscribe((value: FuncionarioDependente[]) => this.setDataList(value))
  }
  private setDataList(value: FuncionarioDependente[]) {
    this.dataList = value;
