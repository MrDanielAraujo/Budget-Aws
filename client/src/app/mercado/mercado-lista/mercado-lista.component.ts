...
@Component({
  selector: 'app-mercado-lista',
  standalone: true,
  imports: [CommonModule, HeaderTableComponent, FooterTableComponent, ReactiveFormsModule],
  templateUrl: './mercado-lista.component.html',
  styleUrls: ['./mercado-lista.component.scss']
})
export class MercadoListaComponent implements OnInit {
  private fb = inject(FormBuilder);
  protected crud = inject(CrudList);
  protected dataList?: Mercado[];
   ...
   */
  ngOnInit(): void {
    this.crud.module = this.crud.sessionKey = "mercado";
  ...
  private actionAfterInitList() {
    this.crud.dataChange.subscribe((value: Mercado[]) => this.setDataList(value))
  }
  private setDataList(value: Mercado[]) {
    this.dataList = value;
